# -*- coding: utf-8 -*-
"""
Created on Sat Jan 06 09:12:13 2018
...
class RegressionTree:# 回归树
    ...
    def chooseBestSplit(self, dataSet, leafType=regLeaf, errType=regErr, ops=(1, 4)):
        tolS = ops[0]  # 容许的误差下降值
        tolN = ops[1]  # 划分的最少样本
        if len(set(dataSet[:, -1].T.tolist()[0])) == 1:  # 类标签的值都是一样的，说明没必要划分了，直接返回  #tolist（）：将数组和矩阵形式转变成列表形式。
...
if __name__ == '__main__':
    ...
    myMat = regTree.loadDataSet('C:/Users/lxl/Desktop/breast cancer.csv')
    myMat = np.mat(myMat)  # 将myMat列表形式转变成矩阵形式
    print(myMat)
